...
3.  **Detail View (`SMM:CreateDetailView`)**:
    ...
    -   **Icon Selection**: A 36x36 icon preview texture and a "Select Icon" button that opens the Icon Browser.
    -   **Body Input**: Large multi-line box for the macro script.
    -   **Actions**:
        -   **New**: Clears inputs to start a fresh macro.
        -   **Save**: Creates a new macro or updates the selected one.
        -   **Delete**: Removes the currently selected macro.
...
| Mode | Description |
| :--- | :--- |
| `"create"` | Clears the form for creating a new macro. Disables Delete button. |
...
└── DetailFrame (Frame)
    ...
    ├── NameLabel & NameEdit
    ├── IconLabel, IconPreview (Texture) & SelectIconButton (Button)
    └── BodyBackdrop (Frame)
